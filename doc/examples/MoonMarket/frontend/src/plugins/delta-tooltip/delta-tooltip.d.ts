...
declare enum LastPriceAnimationMode {
	/**
...
declare enum MismatchDirection {
	/**
...
declare enum TrackingModeExitMode {
	/**
...
declare class MultiTouchCrosshairPaneView implements ISeriesPrimitivePaneView {
...
declare class DeltaTooltipPaneView implements ISeriesPrimitivePaneView {
...
export interface MultiTouchChartOptions {
	simulateMultiTouchUsingMouseDrag: boolean;
}
export type UnSubscriber = () => void;
declare class MultiTouchChartEvents {
	_chartElement: HTMLDivElement;
	_chart: IChartApi;
	_options: MultiTouchChartOptions;
	_mouseState: MouseState;
	private _touchLeave;
	private _touchInteraction;
	_unSubscribers: UnSubscriber[];
	constructor(chart: IChartApi, options: MultiTouchChartOptions);
	destroy(): void;
	leave(): ISubscription;
	move(): ISubscription<MultiTouchInteraction>;
	_addMouseEventListener(target: HTMLDivElement, eventType: "mouseleave" | "mousemove" | "mousedown" | "mouseup", handler: (event: MouseEvent) => void): void;
	_addTouchEventListener(target: HTMLDivElement, eventType: "touchstart" | "touchend" | "touchmove" | "touchcancel", handler: (event: TouchEvent) => void): void;
	_mouseLeave(): void;
	_mouseMove(event: MouseEvent): void;
	_mouseDown(event: MouseEvent): void;
	_mouseUp(): void;
	_touchMove(event: TouchEvent): void;
	_touchFinish(event: TouchEvent): void;
	_touchOther(event: TouchEvent): void;
}
export interface TooltipPrimitiveOptions {
	lineColor: string;
	priceExtractor: <T extends WhitespaceData>(dataPoint: T) => [
		number,
		string
	];
	showTime: boolean;
...
export declare class DeltaTooltipPrimitive implements ISeriesPrimitive<Time> {
	private _options;
	...
	_touchChartEvents: MultiTouchChartEvents | null;
	...
	updateAllViews(): void;
	setData(crosshairData: TooltipCrosshairLineData[], tooltipData: Partial<DeltaTooltipData>): void;
	currentColor(): string;
	chart(): IChartApiBase<Time> | undefined;
	series(): ISeriesApi<keyof SeriesOptionsMap, Time, WhitespaceData<Time> | CustomData<Time> | CustomSeriesWhitespaceData<Time> | LineData<Time> | CandlestickData<Time> | BarData<Time> | AreaData<Time> | BaselineData<Time> | HistogramData<Time>, CustomSeriesOptions | BarSeriesOptions | CandlestickSeriesOptions | AreaSeriesOptions | BaselineSeriesOptions | LineSeriesOptions | HistogramSeriesOptions, DeepPartial<AreaStyleOptions & SeriesOptionsCommon> | DeepPartial<BaselineStyleOptions & SeriesOptionsCommon> | DeepPartial<BarStyleOptions & SeriesOptionsCommon> | DeepPartial<CandlestickStyleOptions & SeriesOptionsCommon> | DeepPartial<HistogramStyleOptions & SeriesOptionsCommon> | DeepPartial<LineStyleOptions & SeriesOptionsCommon> | DeepPartial<CustomStyleOptions & SeriesOptionsCommon>> | undefined;
	applyOptions(options: Partial<TooltipPrimitiveOptions>): void;
	activeRange(): ISubscription<ActiveRange | null>;
	private _setCrosshairMode;
	private _hideTooltip;
	private _hideCrosshair;
	private _chartBackgroundColor;
	private _seriesLineColor;
